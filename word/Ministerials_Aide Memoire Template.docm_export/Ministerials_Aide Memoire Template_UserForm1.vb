...
Private Sub cmbOK_Click()
    ...
    If rg.InlineShapes.Count > 0 Then
        ...
        For Each sp In rg.InlineShapes
            ...
            If sp.Title <> "" Or sp.Title <> " " Then
                Debug.Print sp.Title
                If InStr(LCase(sLogos), LCase(sp.Title)) > 0 Then
                    sp.Range.Font.Hidden = False
                ElseIf InStr(LCase(sOthers), LCase(sp.Title)) > 0 Then
                    sp.Range.Font.Hidden = False
                Else
                    sp.Range.Font.Hidden = True
                End If
                If sp.Range.Font.Hidden = False Then
                    sp.LockAspectRatio = msoTrue
...
Private Sub UserForm_Initialize()
    ...
    If ThisDocument.Bookmarks.Exists(LOGOBOOKMARK2) Then
        Set rg = ThisDocument.Bookmarks(LOGOBOOKMARK2).Range.Cells(1).Range
    End If
    Dim sp As InlineShape
    BBCount = 0
    Dim entries(1 To LOGOCOUNT, 1 To 2) As String
    For Each sp In rg.InlineShapes
        If sp.Title <> "Fis" And sp.Title <> "For" Then
            BBCount = BBCount + 1
            entries(BBCount, 2) = sp.Title
            entries(BBCount, 1) = sp.AlternativeText
        End If
    Next
    'sort array
    Dim aTmp(1 To 1, 1 To 2) As String
    For i = 1 To LOGOCOUNT
        For j = i + 1 To LOGOCOUNT
